...
describe('Widget', () => {
  ...
  describe('nextPosition', () => {
    test('should add goblin-image class to a random field', () => {
      const fields = document.querySelectorAll('.field');
      const initialGoblinCount = Array.from(fields).filter(field => 
        field.classList.contains('goblin-image')
      ).length;
      
      expect(initialGoblinCount).toBe(1);
      
      const goblinField = Array.from(fields).find(field => 
        field.classList.contains('goblin-image')
      );
    ...
    test('should remove goblin-image from previous position', () => {
      ...
      const fields = document.querySelectorAll('.field');
      const hasOldGoblin = fields[firstPosition]?.classList.contains('goblin-image');
    ...
    test('should always generate a different position', () => {
      ...
      // Мокаем так, чтобы возвращал то же число
      let callCount = 0;
      Math.random = jest.fn(() => {
        callCount++;
        // После первого вызова возвращаем другое число, чтобы избежать бесконечного цикла
        if (callCount === 1) {
    ...
    test('should have exactly one goblin-image class at any time', () => {
      for (let i = 0; i < 20; i++) {
        widget.nextPosition();
        const fields = document.querySelectorAll('.field');
        const goblinCount = Array.from(fields).filter(field => 
          field.classList.contains('goblin-image')
        ).length;
  ...
  describe('Edge cases', () => {
    ...
    test('should handle rapid nextPosition calls without errors', () => {
      // Проверяем, что после первого вызова позиция меняется
      const firstPosition = widget.position;
      widget.nextPosition();
      expect(widget.position).not.toBe(firstPosition);
      
      // Продолжаем быстрые вызовы - они все должны работать без ошибок
      for (let i = 0; i < 9; i++) {
        expect(() => widget.nextPosition()).not.toThrow();
      }
    ...
    test('should handle many consecutive calls', () => {
      // Просто проверяем, что 50 вызовов не вызывают ошибок
      for (let i = 0; i < 50; i++) {
        widget.nextPosition();
      }
